...
- 教程主题：HTML / CSS / JS 基础（基础篇全程教程，MDN 降为「课外阅读」）
  + Bootstrap 5.3.8 + jQuery 4.0.0
...
6. **文档同步纪律（重要）**：`.agents/` 与 `AGENTS.md` 必须时刻与项目实际保持一致，
   **实事求是**——写文档前先读真实文件核实，禁止凭记忆或臆测编造。
   - 每完成一批工作，**立即**把该批记录归档为 `notes/progress/<编号>.<英文名>.md`
     （编号从 1 起递增、只增不改；文件名用英文、内容用中文），并同步更新
     `notes/progress.md` 的当前待办与归档索引
     （因用户指令调整，2026-09-14：progress.md 只留当前待办）；
...
| `progress.md` + `progress/` | `progress.md` 只放**当前待办**；已完成批次按编号归档（`progress/1.xxx.md`、`progress/2.xxx.md`…，文件名用英文） | 接手长期任务时：先看待办，再按需查归档 |
